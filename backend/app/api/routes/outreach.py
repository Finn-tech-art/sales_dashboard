...
def get_outreach_status(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[OutreachLog]:
    query = (
        select(OutreachLog)
...
def trigger_outreach(
    ...
    lead = db.scalar(
        select(Lead).where(Lead.id == request.lead_id, or_(Lead.user_id == current_user.id, Lead.user_id.is_(None)))
    )
    if lead is None:
